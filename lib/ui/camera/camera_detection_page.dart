...
import '../../core/services/fruit_classifier_service.dart';
...
class _CameraDetectionPageState extends State<CameraDetectionPage> {
  ...
  final _classifier = FruitClassifierService.instance;
    ...
    try {
      ...
      if (result.topConfidence < AppConfig.minConfidenceToAccept) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'No known fruit detected. Please ensure the fruit is clearly visible, well-lit, and fills the frame.',
            ),
          ),
        );
        setState(() {
          _isCapturing = false;
          _detectedClass = 'No fruit detected';
          _confidence = 0;
          _scores = [];
        });
        return;
      ...
      if (result.topIndex != selectedIndex) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Detected fruit does not match the selected fruit. Please try again with the correct fruit.',
            ),
          ),
        );
        setState(() {
    ...
    try {
      ...
      if (result.topConfidence < AppConfig.minConfidenceToAccept) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'No known fruit detected in the image. Please try another image with better lighting or angle.',
            ),
          ),
        );
        setState(() {
      ...
      if (result.topIndex != selectedIndex) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Detected fruit does not match the selected fruit. Please try another image.',
            ),
          ),
        );
        setState(() {
